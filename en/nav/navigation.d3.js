function navigation(){ 
    ...
    d3.json("nav/adamsmithnav.json", function(error, data) {
        ...
        root_node
            .append("image")
            .attr("xlink:href", "adam-smith.jpg")
            .attr("height", default_circle_root_size)
            .attr("x", - default_circle_root_size/2 - 33)
        ...
        var whole_graph = g.selectAll(".node").filter(function(d, i){ return d.data.position === "root"? false : true; }).on("click", function(d) {
            ...
            window.open("adam-smith/nav/#" + decodeURIComponent(d.data.name), "_self", false);
        });
...
function navigation_subgraph(svg, subgraph){ 
    ...
    d3.json("nav/adamsmithnav-subnodes.json", function(error, data) {
        ...
        var whole_graph = g.selectAll(".node").on("click", function(d) {
            ...
            window.open("adam-smith/search/" + decodeURIComponent(d.data.label.replace(/ /g, "+") + query_variants), "_self", false);
        });
...
$(function(){
    $("#back-to-graph").click(function(){
            window.open("adam-smith/nav/", "_self", false);
    });
});
